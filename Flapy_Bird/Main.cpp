...
public:
	void SetDefaultValues(const std::string path, float _scaleX, float _scaleY);
...
int main()
{
	...
	bird.AddAnim("bird_cadres/bird.png", "fly", 3);
	bird.SetScale(0.3, 0.3);
	bird.SetPosition(50, 350);

	bird.width = bird.scale.x * 250;
	...
	{
		towers[i].SetDefaultValues("bird_cadres/Tower.png", 0.7, 0.7);
		towers[i].position.x = WIDTH;
		towers[i].position.y = Rand(HEIGTH - (int)towers[i].height, HEIGTH - 120);
	}
...
void Tower::SetDefaultValues(const std::string path, float _scaleX, float _scaleY)
{
...
void ControllTowers(Tower towers[], int count, float distance, int* firstTower, float dt, float vy)
{
	...
	if (WIDTH - (towers[lastTower].position.x + towers[lastTower].weight) >= distance)
	{
		if (lastTower == count - 1) lastTower = 0;
		else                        lastTower++;

		towers[lastTower].position.x = WIDTH;
		towers[lastTower].moving = true;

		towers[lastTower].position.y = Rand(HEIGTH - (int)towers[lastTower].height, HEIGTH - 120);
	}
...
bool CheckCollision(Object2D* person, Tower* tower)
{
	...
	else if (person->position.y + person->height > HEIGTH) person->position.y = HEIGTH - person->height;

	if (person->position.y <= tower->towerUp.getPosition().y ||
		person->position.y + person->height >= tower->towerDown.getPosition().y)
	{
		if (person->position.x + person->width >= tower->position.x &&
